...
import { GOOGLE_API_KEY, YOUTUBE_SEARCH_API } from "./utils/constrans";
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(
        `https://youtube.googleapis.com/youtube/v3/search?part=snippet&q=${keyword}&type=video&maxResults=30&regionCode=IN&key=` +
          GOOGLE_API_KEY
      );
  ...
  return searchResult.length === 0 ? (
    ...
    <div className="absolute top-20 overflow-y-auto flex flex-wrap -z-20  h-[88vh] md:top-24">
      ...
      {searchResult.map((val) => (
        <Link to={"/watch?v=" + val.id.videoId} key={val.id}>
          <VideoCards info={val} />
        </Link>
      ))}{" "}
    </div>
